defmodule CpuMonitor do
  ...
  defmap(:start_time, :bpf_array, [max_entries: 32768])
  defmap(:total_time, :bpf_array, [max_entries: 32768])
  ...
  def main(ctx) do
    ...
    if prev_pid > maxEntries do
      Honey.BpfHelpers.bpf_printk(["PID: %d; CPU Time (ns): %llu;", prev_pid, 0])
      return()
    end
    if next_pid > maxEntries do
      Honey.BpfHelpers.bpf_printk(["PID: %d; CPU Time (ns): %llu;", next_pid, 0])
      return()
    end
    ...
    if total do
      ...
      Honey.BpfHelpers.bpf_map_update_elem(:total_time, prev_pid, delta)
    end
