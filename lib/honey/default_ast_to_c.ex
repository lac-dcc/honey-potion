defmodule Honey.CoreElixirToC do
  ...
  # Variables
  def default_ast_to_c({var, var_meta, var_context}, _context)
  ...
  # Blocks
  def default_ast_to_c({:__block__, _, [expr]}, context) do
    Translator.honeys_ast_to_c(expr, context)
  end

  def default_ast_to_c({:__block__, _, _} = ast, context) do
    block = block_to_c(ast, context)
    %TranslatedCode{block | code: "\n" <> block.code <> "\n"}
  end

  # Erlang functions
  def default_ast_to_c(ast = {{:., _, [:erlang, function]}, _, [constant]}, _context)
  ...
  def default_ast_to_c({{:., _, [:erlang, function]}, _, [lhs, rhs]}, context) do
    func_string =
      case function do
        :+ ->
          "Sum"
  ...
  # C libraries
  def default_ast_to_c({{:., _, [Honey.Bpf_helpers, function]}, _, params}, context) do
    case function do
      :bpf_printk ->
  ...
  # Honey Helpers
  # C libraries
  def default_ast_to_c({{:., _, [Honey.Helpers, function]}, _, params}, _context) do
    case function do
      :parse_ethhdr ->
        [data] = params
        typeset = TypeSet.get_typeset_from_var_ast(data)

        cond do
          TypeSet.has_unique_type(typeset, ElixirType.new(:type_ctx_data)) ->
            result_var = Translator.unique_helper_var()

            """
            void *end = (void *)(long)ctx_0_->data_end;
            void *data = (void *)(long)ctx_0_->data;
            struct ethhdr *#{result_var} = data;
            """
            |> gen()
            |> TranslatedCode.new(result_var)

          true ->
            raise "Translator: Currently, the variable passed to Honey.Helpers.parse_ethhdr must be the ctx.data. With this code, we cannot guarantee that."
        end
    end
  end

  # Here for future possibility of global variables. Incomplete.
  def default_ast_to_c({{:., _, [Honey.Bpf.Global, function]}, _, _params}, _context) do
    case function do
  ...
  # def default_ast_to_c({{:., _, [var, property]}, _, _}, _context) do
  ...
  # function raise/1
  def default_ast_to_c({:raise, _meta, [msg]}, _context) when is_bitstring(msg) do
  ...
  # Match
  def default_ast_to_c(pm_operation = {:=, _, [_lhs, _rhs]}, context),
    do: match_operator_to_c(pm_operation, context)

  # Cond
  def default_ast_to_c({:cond, _, [[do: conds]]}, context) do
  ...
  def default_ast_to_c({:{}, _, tuple_values}, context) when is_list(tuple_values) do
    tuple_translations_to_c = Enum.map(tuple_values, fn element -> Translator.honeys_ast_to_c(element, context) end)
  ...
  def default_ast_to_c({first_element, second_element}, context) do
  ...
  def default_ast_to_c([], _context) do
  ...
  def default_ast_to_c([{:|, _meta, [head_element, tail_assignments]}], context) do
  ...
  def default_ast_to_c([first_element | tail], context) do
  ...
  # Case
  def default_ast_to_c({:case, _, [case_input, [do: cases]]} = _case_exp, context) do
  ...
  # Other structures
  def default_ast_to_c(other, _context) do
