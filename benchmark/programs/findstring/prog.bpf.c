...
int __always_inline patternMatch(unsigned int payload_size, struct udphdr *udp, void *data_end) {
    int match;
    for (unsigned int i = 0; i < MAPSTRINGSSIZE; i++) {
        match = TRUE;
        unsigned int key = i;
        ...
        if (ptr && ptr->text[0]) {        
            ...
            if (payload_size != 10L) 
                continue;
            char fmt[] = "String: %s";
            bpf_trace_printk(fmt, sizeof(fmt), ptr->text);
            unsigned int m;
            char pac;
            #pragma clang loop unroll(full)
            for (m = 0; m < SIZETEXT; m++) {
                pac = ptr->text[m];
                if (pac == '\n' || pac == '\0') {
                    break;
                }
            ...
            char fmt2[] = "Passou -> %d\n";
            bpf_trace_printk(fmt2, sizeof(fmt2), (m + 1));
            ...
            for (k = 0; k < payload_size; k++) {
                ...
                int matchIsSpecial = checkSpecialChar(ptr->text[k]);
                if ((k == payload_size - 1) && payloadIsSpecial == TRUE && matchIsSpecial == TRUE)
                    continue;
                else if (payload[k] != ptr->text[k]) {
                    match = FALSE;
                    continue;
                }
            }
