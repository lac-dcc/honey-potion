...
#include "If_Then_Else.skel.h"

void output();

int main(int argc, char **argv) {
  struct If_Then_Else_bpf *skel;
  int err;

  skel = If_Then_Else_bpf__open();
  if(!skel){
    fprintf(stderr, "Skeleton failed opening.\n");
    return 1;
  }

  err = If_Then_Else_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    If_Then_Else_bpf__destroy(skel);
    return -err;
  }

  err = If_Then_Else_bpf__attach(skel);
  if(err){
