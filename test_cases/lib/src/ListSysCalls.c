...
#include "ListSysCalls.skel.h"

void output();

int main(int argc, char **argv) {
  struct ListSysCalls_bpf *skel;
  int err;

  skel = ListSysCalls_bpf__open();
  if(!skel){
    fprintf(stderr, "Skeleton failed opening.\n");
    return 1;
  }

  err = ListSysCalls_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    ListSysCalls_bpf__destroy(skel);
    return -err;
  }

  err = ListSysCalls_bpf__attach(skel);
  if(err){
