...
#include "Honey_List.skel.h"

void output();

int main(int argc, char **argv) {
  struct Honey_List_bpf *skel;
  int err;

  skel = Honey_List_bpf__open();
  if(!skel){
    fprintf(stderr, "Skeleton failed opening.\n");
    return 1;
  }

  err = Honey_List_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    Honey_List_bpf__destroy(skel);
    return -err;
  }

  err = Honey_List_bpf__attach(skel);
  if(err){
